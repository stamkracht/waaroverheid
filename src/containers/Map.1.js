...
class MapContainer extends React.Component {
    ...
    async componentWillReceiveProps(nextProps) {
        ...
        if (this.state.code !== code) {
            ...
            let { facets, meta: { total: documentsCount } = 0, events: documents = [] } = await SearchService.search(
                code
            );
    ...
    async handleOnSubmitSearch(query, filters) {
        const { facets, meta: { total: documentsCount } = 0, events: documents = [] } = await SearchService.search(
            this.state.code,
            query
        );
        const page = 1;
        ...
        this.setState({ query, facets, documentsCount, documents, filters, page, hasMoreDocs }, () =>
            this.handleRouting(this.props.match.params.code)
        );
    }
    ...
    async getMoreDocuments(page) {
        ...
        FiltersService.set(filters);
        let { meta: { total: documentsCount } = 0, events: documents = [] } = await SearchService.search(
            this.state.code,
            this.state.query,
            page
        );
    ...
    renderFilters() {
        if (!this.state.drawerActive && this.state.documentsCount) {
            return (
                <div>
                    <Filters facets={this.state.facets} submit={this.handleOnSubmitSearch.bind(this)} />
                </div>
            );
        }
    }
        ...
        return (
            <div>
                <NavigableMap
                    ...
                    code={this.state.code}
                    counts={MapService.getAreaCounts(this.state.facets, this.state.code, this.state.documentsCount)}
